...
describe('POST /translate/normal', () => {
  it('success when valid text is supplied', async () => {
    ...
    expect(response.body.translation).toEqual(
      "AoArore yoyou tothohe cocatot'sos owownoneror?"
    );
  });

  it('fail when valid no text or empty string is supplied', async () => {
    await request(app).post('/translate/normal').send().expect(400);
...
describe('POST /translate/rovarspraket', () => {
  ...
  it('fail when valid no text or empty string is supplied', async () => {
    await request(app).post('/translate/rovarspraket').send().expect(400);
...
describe('POST /translate/translateJokeOfTheDay', () => {
  it('success against a valid request', async () => {
    await request(app).post('/translate/translateJokeOfTheDay').expect(200);
  });
});
